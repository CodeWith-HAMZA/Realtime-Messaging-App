...
// body - otherUserId
router.route("/access-chat").post(authenticateToken, async function (req, res, next) {
    ...
    if (isChat.length > 0) {
        ...
        const chatWithPopulatedUser = await Chat.findById(createdChat._id).populate('users', '-password')
        return res.status(200).json({ chat: chatWithPopulatedUser })
...
router.route("/all").get(authenticateToken, async function (req, res) {
    ...
    try {
        ...
        const chats = await Chat.find({ users: { $all: [currentUser] } }).populate({ path: 'users', select: 'name email' }) // Populate 'users' field with 'name' and 'email' properties
...
router.route("/group").post(authenticateToken, async function (req, res) {
    try {
        ...
        if (parsedUsers.length < 2) {
            return res.status(400).json({ message: 'At least 3 Users Exist To Form A Group Chat' });
        }
        ...
        if (existingGroup) {
            return res.status(400).json({ message: 'Group chat with the same name already exists' });
        }
        ...
        return res.status(201).json({ groupChat: populatedGroupChat, message: 'successfully created group' });
    } catch (error) {
        console.error('Error:', error);
...
router.route("/rename").put(authenticateToken, async function (req, res) {
    try {
        ...
        return res.json({ groupChat: updatedGroupChat, message: 'Group chat renamed successfully' });
    } catch (error) {
        console.error('Error:', error);
...
router.get("/group-chats", authenticateToken, async function (req, res) {
    try {
        ...
        }).populate('users', 'name email').populate('groupAdmin', 'name email');
...
router.route("/group/add").put(authenticateToken, async function (req, res) {
    try {
        ...
        if (!groupChat.users.includes(userId)) {
            ...
            return res.status(400).json({ message: "already the member of the group" })
        }

        // Save the updated group chat
        const updatedGroupChat = await groupChat.save();

        return res.json({ groupChat: updatedGroupChat, totalUsers: updatedGroupChat?.users?.length, message: 'User added to the group successfully' });
    } catch (error) {
        console.error('Error:', error);
...
router.route("/group/remove").put(authenticateToken, async function (req, res) {
    try {
        ...
        return res.json({ groupChat: updatedGroupChat, totalUsers: updatedGroupChat.users.length, message: 'User removed from the group successfully' });
    } catch (error) {
        console.error('Error:', error);
...
router.route("/group/make-admin").put(authenticateToken, async function (req, res) {
    try {
        const { groupId, userId } = req.body;
        const currentUser = req.user._id;
        ...
        if (typeof groupId !== "string" || typeof groupId !== 'string') {
            return res.status(400).json({ error: 'Both Group-id, and user-id must be a pure string-ids' });
        }
        ...
        // Check if the current user is the group admin
        if (groupChat.groupAdmin.toString() !== currentUser) {
            return res.status(403).json({ message: 'Only the group admin can promote users to admin' });
        }
        ...
        return res.json({ groupChat: updatedGroupChat, message: 'User promoted to admin successfully' });
    } catch (error) {
        console.error('Error:', error);
